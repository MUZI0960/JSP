...
<div>
	...
	<input type="radio" name="contentType" checked value="application/json"/>JSON(Error)
...
<div>
	...
	<input type="radio" name="dataType" value="xml"/>XML(Error)
	<input type="radio" name="dataType" value="html"/>HTML
</div>
<form id="calForm" action="<%=request.getContextPath() %>/calculator/case7" method="post">
...
$("#calForm").on("submit", function(event) {
	...
	$.ajax({
		...
		if(jqXHR.responseJSON){
			resultArea.text(resp['calculateModel']['expression']);
